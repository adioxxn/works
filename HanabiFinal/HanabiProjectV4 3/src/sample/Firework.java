...
public class Firework {
    ...
    public Sparcle[] sparcles;//a firework contain many sparcles
    ...
    public void update() {
        if (this.exploded) {
            ...
            for (int i = 0; i < sparcles.length; i++) {
                if (sparcles[i] != null) {//if the sparcles is not null, keep exploded it
                    this.sparcles[i].update();
                    //if it is null, remove it from the list
            ...
            if (amountOfSparcleDead == this.sparcles.length) {
                this.dead = true;//if all the sparcles dead, the firework dead
            }
        }else{
            this.fuse--;
            if(this.fuse<=0){
