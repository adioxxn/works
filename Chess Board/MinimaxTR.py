# CMPT 317 A Python implementation of Game Tree Search - Minimax with Transposition table
...
#    transposition_string(self, state)
#       - returns a unique string representation of the state 
#       - for use as a key in a transposition table (key is a string, value is the Minimax value)

# To use the MinimaxTR class, you need the following steps
#    game = <create a game object from a Game Class>
#    searcher = MinimaxTR(game)         # create an instance of Minimax for the game
...
class MinimaxTR(object):
    ...
    def minimax_decision_max(self, state):
        ...
        for act in self.game.actions(state):
            val = self.__min_value(self.game.result(state, act))
            if val > best:
    ...
    def minimax_decision_min(self, state):
        ...
        for act in self.game.actions(state):
            val = self.__max_value(self.game.result(state, act))
            if val < best:
                # remember something better
                best = val
                best_action = act
