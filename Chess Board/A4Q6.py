...
class SearchTerminationRecord(object):
    ...
    def __init__(self, value, move, table, time=0, nodes=0, cutoff=False):
    ...
    def __str__(self):
        ...
        """
        text = 'Chose move <{}> with {} Minimax value {} after {:.4f} seconds, expanding {} nodes, table size= {}'
        if self.cutoff:
            textsuccess = 'estimated'
        else:
            textsuccess = 'true'
        return text.format(self.move, textsuccess, self.value, self.time, self.nodes, len(self.table))
...
class AlphaBeta(object):
    ...
    def minimax_decision_max(self, state):
        ...
        return SearchTerminationRecord(best, best_action, self.transposition_table, end - start, self.nodes_expanded)
